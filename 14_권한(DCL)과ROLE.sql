...
--테이블 스페이스
-- : 테이블, 뷰, 그밖의 데이터 베이스 객체들이 저장되는 디스크상의 장소
SELECT * FROM DBA_USERS
WHERE USERNAME ='SAMPLE';

--WITH ADMIN OPTION
-- 사용자에게 시스템권한을 부여할 때 사용함
--권한을 부여 받은 사용자는 다른 사용자에게 권한을 지정할 수 있음
GRANT CREATE SESSION TO SAMPLE
WITH ADMIN OPTION;--다른 사용자에게 권한을 줄 수 있음
--------------------------------------------------------
...
-- 현재 사용자에게 부여된 권한 조회
SELECT * FROM USER_TAB_PRIVS_RECD;--SAMPLE계정에서
-- 현재사용자가 부여한 권한 조회
SELECT * FROM USER_TAB_PRIVS_MADE;--EMPLOYEE계정;
...
--생성된 롤 조회(시스템계정, SAMPLE계정에서 조회)
SELECT * FROM USER_ROLE_PRIVS
WHERE GRANTED_ROLE='MYROLE';