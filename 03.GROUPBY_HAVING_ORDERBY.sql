...
--직원 테이블에서 부서코드 별 그룹을 지정하여
--부서코드, 그룹별 급여의 합계, 그룹별 급여의 평균(정수처리-FLOOR),인원수 조회
--부서 코드 순으로 정렬
SELECT
    DEPT_CODE,
    SUM(SALARY) 급여합,
...
SELECT
    ...
    COUNT(*) 사원수
FROM EMPLOYEE
WHERE BONUS IS NOT NULL OR BONUS!=0
GROUP BY JOB_CODE
ORDER BY JOB_CODE;
...
--부서별 그룹의 급여합계 중 구백만원 초과 부서코드와 급여합계조회
SELECT
    DEPT_CODE,
    SUM(SALARY) 합계
FROM EMPLOYEE
...
SELECT
    ...
    CASE
        ...
        WHEN GROUPING(DEPT_CODE) = 0 AND GROUPING(JOB_CODE)=0 THEN '부서별 합계'
        ELSE '총합'
    END 구분
FROM EMPLOYEE
GROUP BY CUBE(DEPT_CODE, JOB_CODE)
...
--INTERSECT : 여러개의 SELECT한 결과에서 공통된 부분만 결과로 추출
--수학에서 교집합
...
--MINUS : 선행 SELECT결과에서 다음 SELECT 결과와 겹치는 부분을 제외한 나머지 부분만 추출
--수학에서 차집합
SELECT
    EMP_ID,
    EMP_NAME,
...
SELECT
    ...
    FLOOR(AVG(SALARY))
FROM EMPLOYEE
GROUP BY DEPT_CODE,JOB_CODE, MANAGER_ID --15행
UNION ALL
SELECT
    DEPT_CODE,
    ''JOB_CODE,
...
--GROUPING SETS : 그룹별로 처리된 여러개의 SELECT문을 하나로 합칠 때 사용한다.
--SET OPERATION(집합연산)에서 UNION ALL과 동일
SELECT
    DEPT_CODE,
    JOB_CODE,
    MANAGER_ID,
