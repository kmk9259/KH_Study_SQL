...
--컬럼의 크기 줄이는 경우
--변경하려는 크기를 초과 하는 값이 없을 때만 수정 가능
ALTER TABLE DEPT_COPY2
MODIFY DEPT_TITLE VARCHAR2(10); --cannot decrease column length because some value is too big
...
ALTER TABLE DEPT_COPY2
DROP COLUMN DEPT_ID; --ORA-12983: cannot drop all columns in a table 

CREATE TABLE TB1( --순환 참조 (자기자신 테이블을 참조)
...
--컬럼 삭제시 참조하고 있는 컬럼이 있다면 삭제 못한다.
ALTER TABLE TB1
DROP COLUMN PK; --Drop all constraints referencing the parent key column, or
...
--제약조건 삭제
--ON DELETE CASCADE : 삭제되는 외부키에 해당하는 행을 같이 삭제 (참조무결성제약조건)
