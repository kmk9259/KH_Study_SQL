...
END;
/
--SET SERVEROUTPUT ON을 실행시켜줘야 출력한다. 기본은 OFF
SET SERVEROUTPUT ON;
...
EXCEPTION
    WHEN NO_DATA_FOUND
    THEN DBMS_OUTPUT.PUT_LINE('조회결과가 없음');
END;
/
--정의되지 않은 예외처리
DECLARE
...
EXCEPTION
    WHEN DUP_EMPNO
    THEN DBMS_OUTPUT.PUT_LINE('이미 존재하는 사원');
END;
/
...
DECLARE
    ...
    EMP_ID_TABLE EMP_ID_TABLE_TYPE; --배열선언
...
BEGIN 
    ...
    LOOP
        RESULT := DAN * SU;
         DBMS_OUTPUT.PUT_LINE('DAN :'||DAN||' RESULT : '||RESULT);     
    END LOOP;
