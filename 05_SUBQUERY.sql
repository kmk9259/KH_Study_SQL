...
--부서코드가 노옹철 사원가 같은 소속의 직원 명단을 조회하시오
SELECT
    EMP_NAME
FROM EMPLOYEE
...
--서브쿼리는 select, FROM, WHRER, HAVING, ORDER BY절에 사용 가능
--부서별 급여의 합계가 가장 큰 부서의 부서코드, 부서명, 급여합계
SELECT
    B.DEPT_ID,
    B.DEPT_TITLE,
    SUM(A.SALARY)
...
WHERE SALARY IN (SELECT
                ...
                FROM EMPLOYEE
                GROUP BY DEPT_CODE);
--관리자에 해당하는 지원에 대한 정보와 관리자가 아닌 직원의 정보를 추출하여 조회
--사번, 이름, 부서명, 직급명, '관리자' AS구분/ '직원' 
SELECT
    A.EMP_ID 사번,
    A.EMP_NAME 이름,
    B.DEPT_TITLE 부서명,
...
--J3코드를 가진 사람의 급여들을 가지고 비교 할 때
SELECT
    SALARY
FROM EMPLOYEE
WHERE JOB_CODE ='J3'; --최소3400000, 최대 3900000
...
WHERE SALARY<ANY(SELECT
                ...
                FROM EMPLOYEE
                WHERE JOB_CODE ='J3');
--설명 : < ANY 이므로 서브쿼리에서 줄력된 결과 중에서 최대값보다 작으면 출력
--하나라도 참이면 되기 때문에 최대값보다 작으면 된다
...
WHERE SALARY > ANY(SELECT
                ...
                FROM EMPLOYEE
                WHERE JOB_CODE ='J3');
--설명 : > ANY이므로 서브쿼리에서 출력된 결과 중 최소값 보다 크면 출력
--하나라도 참이면 되기 때문에 최소값 보다 크면 된다.
...
WHERE SALARY<ALL(SELECT
                ...
                FROM EMPLOYEE
                WHERE JOB_CODE ='J3');
--설명 : < ALL이므로 서브쿼리에서 출력 된 결과 중 최소값 보다 작으면 출력
...
WHERE SALARY > ALL(SELECT
                ...
                FROM EMPLOYEE
                WHERE JOB_CODE ='J3');
--설명 : > ALL이므로 서브쿼리에서 출력 된 결과 중 최대값 보다 크면 출력 
--모두 참이어야 하기 때문에 크려면 최대값이 제일 큰 값이므로 최대값 보다 커야한다.
...
--차장 직급의 급여의 가장 큰 값보다 많이 받는 과장직급의 
--사번, 이름, 직급, 급여를 조회
SELECT
    A.EMP_ID 사번,
    A.EMP_NAME 사원명,
...
--다중행 다중열 서브쿼리 이용
--직급별 평균 월급이 직급과 월급 둘 다 일치하는 사원 (만원단위로 계산하세요 TRUNC(컬럼명, -5))
SELECT
    JOB_CODE,
    TRUNC (AVG(SALARY),-5)
FROM EMPLOYEE
...
SELECT
    ...
    NVL((SELECT B.EMP_NAME
     FROM EMPLOYEE B
     WHERE B.EMP_ID = A.MANAGER_ID),'없음') 관리자명 --매니저 ID가 200번이면 성동일
FROM EMPLOYEE A
ORDER BY 1;
...
--JOB 코드별 월급 평균(TRUNC(AVG(E2.SALARY),-5))을 구하고 월급이 이와 일치하는 사원정보 이름, 직급명,월급
SELECT
    A.EMP_NAME,
    C.JOB_NAME,
    A.SALARY
...
JOIN (
        ...
        FROM EMPLOYEE
        GROUP BY JOB_CODE) B ON A.JOB_CODE = B.JOB_CODE   --JOB코드 별 월급 평균
        AND A.SALARY = B.JOBAVG
JOIN JOB C ON A.JOB_CODE = C.JOB_CODE
ORDER BY A.JOB_CODE;

--부서명이 인사관리부인 사원명 부서명 직급이름 구하시오(인라인뷰사용)
--먼저 사원명 부서명 직급이름 주테이블 EMPLOYEE
SELECT A.*
FROM (
        SELECT
            A.EMP_NAME 사원명,
...
ORDER BY SALARY DESC; --ORDER BY 전에 정렬이 되어있다.
...
--급여 평균 3위 안에드는 부서의 부서코드와 부서명, 평균 급여를 조회하세요
SELECT
    A.*
FROM (SELECT
            A.DEPT_CODE,
...
--직원 정보에서 급여를 가장 많이 받는 순으로 이름, 급여, 순위
SELECT
    EMP_NAME,
    SALARY,
    DENSE_RANK() OVER (ORDER BY SALARY DESC) 순위 --순위가 순차적으로 나옴
FROM EMPLOYEE;

SELECT
    EMP_NAME,
    SALARY,
    RANK() OVER (ORDER BY SALARY DESC) 순위 --순위가 순차적이지 않고 중복된 수 만큼 증가해서 나옴
FROM EMPLOYEE;
...
--같은 서브쿼리가 여러번 사용 될 경우 중복 작성을 줄일 수 있다.
...
--부서 별 급여 합계가 전체 급여의 총합의 20% 보다 많은
