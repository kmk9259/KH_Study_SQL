...
INSERT INTO EMPLOYEE
    VALUES(900,'장채현','901123-108050','JANG@kh.or.kr','01055569512','D1','J7','S3',4300000,0.2,'200',SYSDATE,NULL,DEFAULT);
COMMIT;
SELECT * FROM EMPLOYEE
WHERE EMP_NAME='장채현';

--INSERT시에 VALUES 대신 서브쿼리를 이용할 수 있따.
CREATE TABLE EMP_01(
    EMP_ID NUMBER,
...
--방명수 사원의 급여 인상소식을 전해들은 다른 직원들이
--단체로 파업을 진행했다
--노옹철, 전형돈, 정중하, 하동운 사원의 급여와 보너스를
--유재식 사원의 급여, 보너스와 같게 변경하는 UPDATE문
UPDATE EMP_SALARY
...
--EMP_SALARY 테이블에서 아시아근무지역에 근무하는 직원의 보너스를 0.5로 변경
UPDATE EMP_SALARY
SET BONUS =0.5
...
UPDATE EMPLOYEE
SET DEPT_CODE='65' --DEPT_CODE가 D1~D9까지 박에 없어서 FK제약조건에 위배
WHERE DEPT_CODE ='D6';

UPDATE EMPLOYEE
SET EMP_NAME =NULL --EMP_NAME이 NOT NULL제약조건으로 위배된다
WHERE EMP_ID=200;

UPDATE EMPLOYEE
SET EMP_NO='621235-1985634' --UNIQUE 제약조건에 위배됨
WHERE EMP_ID=201;
...
--만약 WHERE절에 조건을 설정하지 않으면 모든 행이 삭제
...
--FK제약조건이 설정 되어 있는 경우 참조되고 있는 값에 대해서는 삭제 할 수 없다.
SELECT DISTINCT DEPT_CODE FROM EMPLOYEE;

DELETE FROM DEPARTMENT WHERE DEPT_ID ='D1';

--FK 제약조건이 설정되어 있는 경우
--참조 되고 있지 않은 값은 삭제 가능
ROLLBACK;

--삭제시 FK제약조건으로 컬럼 삭제가 불가능한 경우
--제약 조건을 비활성화 할 수 있다.
ALTER TABLE EMPLOYEE
DISABLE CONSTRAINT SYS_C007136 CASCADE; 

--TRUNCADE : 테이블의 전체 행을 삭제할 시 사용된다.
--DELETE보다 수행속도가 빠르다
...
--MERGE (M_TEST01에 M_TEST02를 병합)
MERGE INTO M_TEST01 M1 USING M_TEST02 M2 ON (M1.ID=M2.ID)
WHEN MATCHED THEN --위 ON의 걸려있는 값이 ON이면 UPDATE 실행
UPDATE
SET M1.NAME = M2.NAME
WHEN NOT MATCHED THEN --위 ON이 FALSE이면 INSERT를 실행
INSERT (M1.ID, M1.NAME)
VALUES (M2.ID, M2.NAME);
